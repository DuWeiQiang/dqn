...
def getInputImage(num=0):
  plt.imshow(deprocess(net.blobs['frames'].data[num], mean=mean_blob, raw_scale=255))
...
def vis_filters(layer_num=1):
  if layer_num == 1:
    filters = net.params['conv1_layer'][0].data
    vis_square(filters.transpose(0, 2, 3, 1))
  elif layer_num == 2:
    filters = net.params['conv2_layer'][0].data
    vis_square(filters[:16].reshape(16**2, 4, 4))

def vis_activations(layer_name):
